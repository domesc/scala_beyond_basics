...
class AdvancedImplicitsSpec extends FunSpec with Matchers {
  describe(
    ...
    it( """can bring up any implicit directly by merely calling up implicitly""") {
      ...
      implicit val flavorOfTheDay = IceCream("Pirate's Cove")

      def orderFlavorOfTheDay(n: Int) = Scoops(n, implicitly[IceCream])
    ...
    it(
      ...
      implicit val listOfString: List[String] = List("Foo", "Bar", "Baz") // type erasure happens at runtime and not at compile time
  ...
  describe("Locating implicits recipes") {
    ...
    it("""can use JavaConverters to convert a collection in Java to Scala and vice versa""") {
      ...
      val result = ZoneId.getAvailableZoneIds.asScala.toSet.filter(x => x.startsWith("Asia")).map(x => x.split("/").last).size
  ...
  describe("View Bounds are used to ensure that there is a particular recipe for a certain type") {
    it(
      ...
      implicit def str2Employee(s: String) = {
      ...
      // A-> Employee
      def hireEmployee[A <% Employee](s: A) = { // view bound
    ...
    it(
      ...
      object MyPredef {
        implicit val loggableExclamation: Loggable[Employee] = new Loggable[Employee] {
          override def log(t: Employee): String = s"Employee! ${t.firstName} ${t.lastName}"
        }
  ...
  describe(
    ...
    it(
      ...
      //val myHetherogeneousPair = new MyPair[String, Int]("Foo", 10)
      //myHetherogeneousPair.toList
    }
  ...
  describe("Getting around Erasure Using TypeTags") {
    it("used to use Manifest but now uses a type tag to retrieve what is erased") {
      ...
      matchList(List(1,2,3)) should be ("List of Int") // Wrong
    }
  }
  ...
  describe(
    ...
    it(
      ...
      class Employee(val firstName: String, val ln: String)
      ...
      object EmployeePredef {
        implicit val employeeFullNameEquality: Eq[Employee] = new Eq[Employee] {
          override def equals(a: Employee, b: Employee): Boolean = {
            a.firstName == b.firstName && a.ln == b.ln
          }
        }
      ...
      import EmployeePredef.employeeFullNameEquality
      val x = new Employee("Bjarne", "Stroustroup")
      val y = new Employee("Bjarne", "Edvar")

      def equals[A](a: A, b: A)(implicit eq: Eq[A]): Boolean = {
        eq.equals(a, b)
      }

      equals(x, y) should be (false)
    }

    it("can be used for ordering") {
      case class Employee(val firstName: String, val ln: String)

      implicit val employeeOrdering: Ordering[Employee] = new Ordering[Employee] {
        override def compare(x: Employee, y: Employee): Int = y.ln.compareTo(x.ln)
      }
